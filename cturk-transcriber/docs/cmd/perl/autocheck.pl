...
my $searchSth = $dbh->prepare("SELECT id, Answers, StdAnswers FROM cturk_list WHERE status = 2");
$searchSth->execute();
...
while($ref = $searchSth->fetchrow_hashref()){
    ...
    if($@){
        ...
        while ($answers->{"Answers"}[0]->{"Answer"}[$answersLength]) {
            #print $answers->{"Answers"}[0]->{"Answer"}[$answersLength]->{"key"}[0];
            #print "=";
            #print $answers->{"Answers"}[0]->{"Answer"}[$answersLength]->{"value"}[0]."\n";
        ...
        foreach $aValue(keys(%stdAnswersHash)){
            ...
            if(pstr(lc($stdAnswersHash{$aValue})) ne ""){
                my $stdStr = pstr(lc($stdAnswersHash{$aValue}));
                my $userStr = pstr(lc($answersHash{$aValue}));
                if(isApproveHit($stdStr, $userStr)){
                    $isReject = 0;
                }else{
                    $isReject = 1;
                }
    ...
    if($isReject == 1){
        #set status = 4
        my $sql = $dbh->prepare("UPDATE cturk_list SET status = 4 WHERE id=$id");
        $sql->execute();
    }else{
        #set status = 3
        my $sql = $dbh->prepare("UPDATE cturk_list SET status = 3 WHERE id=$id");
        $sql->execute();
    }
}
