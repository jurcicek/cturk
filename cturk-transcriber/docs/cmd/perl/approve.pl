#!/usr/bin/perl5.8
# Auto approve Transcription HITs, but generate NO Validation HITs
...
#   Set HIT's status in the database to 5.
...
my $insertSth = $dbh->prepare(qq{
    INSERT INTO cturk_list2(path, status, StdQuestions, StdAnswers, audioLength)
    VALUES(?, 0, ?, ?, ?)
});
...
my $searchSth = $dbh->prepare("
	...
	FROM cturk_list WHERE status = 3");
$searchSth->execute();
...
sub approveAssignment(){
	...
	if($resp->is_success){
		...
		if($isSuccess eq "1"){
			...
			# set status = 5, then create a new hit2
			...
			#$insertSth->execute($path, $stdQuestions2, $stdAnswers2, $audioLength);
			#$dbh->commit;
			
			#TODO exec loadhit2
		}else{
			print "[ERROR] approve error: id=$id\n";
		}
